...
- 山石/Array 厂商驱动从骨架升级为官方手册+生产真机双验证：山石
  ...
  `^%\s`(%后空白排除日志行)
- 锐捷厂商驱动经官方手册核对(RGOS 11.4(1)B42 + 12.5/12.6 系列四份，共5份配置指南)：
  保存命令修正为 `write`(原 write memory 无官方证据)；提示符体系、错误样式(与
  Cisco IOS 一致)、分页命令 `terminal length`(12.x 收录，缺省单屏24行)均获官方确认
